  ...
  <div class="z-20 flex text-font">
    ...
    <div
      ...
      <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1"
        ...
        <div v-show="isOpen"
          class="m-0 fixed top-24 2xl:right-48 bottom-auto 2xl:left-48 left-20 right-20  z-30 rounded-md bg-white shadow-lg ring-1 mt-3 
                  ...
                  <div v-for="panel, index in item.megaMenuContent?.panels" :key="index" class="tabpanel"
                    ...
                    <div class="grid grid-cols-4 divide-x divide-slate-200 dark:divide-slate-700">
                      <div v-for="subPanel, index in panel.subPanels" :key="index" class="ps-6">
                        <h6 class="text-base font-medium text-slate-800 dark:text-slate-200">
                          {{ subPanel.title }}</h6>
                        <ul class="grid space-y-3 mt-4">
                          <li v-for="item, index in subPanel.items" :key="index">
                            <a class="text-sm font-medium text-slate-600 hover:text-primary dark:text-slate-400" :href="item.href">{{
                              item.name }}</a>
                          </li>
                        </ul>
                      </div>
