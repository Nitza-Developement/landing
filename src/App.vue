<script setup lang="ts">
import Card from './components/Card.vue';
import type { CardContent } from './types/types';
import { ref } from 'vue'

const bgColor = ref("");
const textColor = ref("");

const changeBackground = () => {
  window.document.body.style.backgroundColor = "";
  if (bgColor.value === "bg-transparent") {
    bgColor.value = "bg-slate-700";
    textColor.value = "text-white";
    window.document.body.classList.remove("bg-transparent");
    window.document.body.classList.add("bg-slate-700");
  } else {
    bgColor.value = "bg-transparent";
    textColor.value = "";
    window.document.body.classList.remove("bg-slate-700");
    window.document.body.classList.add("bg-transparent");
  }

}
changeBackground();


const cardsContent: CardContent[] = [
  {
    imagenLink: "data:image/png;base64,",
    title: "Fast Foods",
    text: "Healthy Foods are nutrient-Dense Foods",
  },
  {
    imagenLink: "https://coderthemes.com/yum/assets/vegetables-5e491677.png",
    title: "Fast Foods",
    text: "Healthy Foods are nutrient-Dense Foods",
  },
  {
    imagenLink: "data:image/png;base64,",
    title: "Fast Foods",
    text: "Healthy Foods are nutrient-Dense Foods",
  },
]
</script>

<template>
  <button @click="changeBackground()">MODO OSCURO</button>
  <div class="flex gap-8 p-8 justify-center">
    <Card :cardContent="cardsContent[0]" :bgColor="bgColor" :textColor="textColor" />
    <Card :cardContent="cardsContent[1]" :bgColor="bgColor" :textColor="textColor" />
    <Card :cardContent="cardsContent[2]" :bgColor="bgColor" :textColor="textColor" />
  </div>
</template>
